...
outfile ='{g}_AMR_plus_metal_gene_counts.csv'.format(g=args.genus)
    ...
    for genus in generalist:
        ...
        for source in sourcelist:
            ...
            for gene in arglist:
                ...
                for mrg in metallist:
                    countarg = inputdata.query('scientific_name.str.contains("{g}", na=False) & Source=="{s}" '
                                               '& AMR_genotypes.str.contains("{arg}", na=False) & stress_genotypes.str.contains("{mrg}", na=False)'.format(g=genus,s=source,arg=gene,mrg=mrg))['scientific_name'].count()
                    with open(outfile, 'a+') as file:
                        file.write("{g},{cg},{s},{arg},{mrg},{counts},{ao},{countarg}\n".format(g=genus,cg=counttotal,s=source,arg=gene,mrg=mrg,ao=countg,counts=countsource,countarg=countarg))
...
for gene in arglist:
    ...
    for mrg in metallist:
        countcarg = inputdata.query('scientific_name.str.contains("{g}", na=False) & epi_type=="clinical" & host=="Homo sapiens" '
                                   '& AMR_genotypes.str.contains("{arg}", na=False) & stress_genotypes.str.contains("{mrg}", na=False)'.format(g=genus,arg=gene,mrg=mrg))['scientific_name'].count()
        with open(outfile, 'a+') as file:
            file.write("{g},{cg},Clinical,{arg},{mrg},{counts},{ao},{countarg}\n".format(g=genus,cg=counttotal,arg=gene,mrg=mrg,ao=countg,counts=clinicalsource,countarg=countcarg))
